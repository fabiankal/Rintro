...
#install.packages("panelr")# remember installing if not done previously
...
- Panel regression with fixed effects (utilizing changes instead of levels)
...
In addition it is impracticable to have different variable names for the same variable. For example the variable "Finanizielles Auskommen" in wave 6 and 7 differs not only because of the prefix "W6_" "W7_" but also because the rest is different: "W6_Q72","W7_Q69". In this form we cannot easily switch between long and wide data and R does not know which colums contain similar variables measured in different waves. 
...
```{r renaming variables and reshaping the data}
#note you can select multiple lines at once if you hold the alt key and then select lines by dragging with the mouse to create a selection
...
ggplot(data_mean, aes(color=as.factor(ase_sum_n),ase_sum_n,x=as.factor(wave), y=mean)) + #color instead of fill because points have colors and no fill
    ...
    geom_errorbar(aes(ymin=ymin,ymax=ymax),width=.1, show.legend = FALSE) + # add sescond geometric shape in form of errorbars in addition to the points indicating the means
    labs(y = "Durchschnittliches finanzielles Auskommen\n0=sehr schwer - 4=sehr gut", x= "Welle",color="Vermögenswerte (Summenindex)") + 
...
Finally, we can move to actually testing our hypotheses using panel regression with fixed effects in different forms. First we test H1, then H3. Our estimatgion strategy uses a linear regression with an ordinary least squares estimator (OLS) and clustered standard errors. We export the results in form of a regression table, a coefficient plot and a plot indicating Average marginal effects. 
...
#We now try to calculate a similar model using the fixest package. This package will afterwards allow us to account for the panel structure by calculating regressions with fixed effects. 
...
We investigate whether the effect of Kurzarbeit changes due to a reform starting at the first of October 2020 (Kurzarbeit Phase 3). 
...
#regressions for the dif in dif model
